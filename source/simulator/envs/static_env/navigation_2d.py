...
from src.envs.static_env.obstacle_map_2d import ObstacleMap, generate_random_obstacles
from src.dynamics.bicycle_like_dynamics import Bicycle_Dynamics

BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
setting_path = os.path.join(BASE_DIR, 'envs/static_env/setting.yaml')
...
class Navigation2DEnv:
    def __init__(
        ...
        self._start_pos = torch.tensor(
            [-(param['map_size']/2-20), -(param['map_size']/2-20)], device=self._device, dtype=self._dtype
        )
    ...
    def step(self, u: torch.Tensor) -> Tuple[torch.Tensor, bool]:
        ...
        ) # why this type is tensor?
        is_goal_reached = bool(is_goal_reached.any().item())
    ...
    def render(
        ...
        # visualize collision effect
        if is_collisions is not None:
            if is_collisions[0, 0].item() > 0:
                circle = patches.Circle((self._robot_state[0].cpu().numpy(), self._robot_state[1].cpu().numpy()),
        ...
        if predicted_trajectory is not None:
            ...
            if is_collisions is not None:
                is_collisions = is_collisions.cpu().numpy()    # numpy is only operated on cpu architecture
    ...
    def cost_function(self, state: torch.Tensor, action: torch.Tensor, info: dict) -> torch.Tensor:
        ...
        # to get sensory range, we take initial robot state 
        ...
        cost = 2 * goal_cost + 1 * velocity_cost + 10000*obstacle_cost
    ...
    def collision_check(self, state: torch.Tensor) -> torch.Tensor:
        ...
        """
        pos_batch = state[:, :, :2]
        is_collisions, _ = self._obstacle_map.compute_cost(pos_batch) # .squeeze(1)
        is_collisions = is_collisions.squeeze(1)   # remove axis 1 => [[x],[y]] -> [x,y]
