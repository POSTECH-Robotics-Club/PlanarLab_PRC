...
setting_csv_path = os.path.join(BASE_DIR, 'src/envs/track_generator/path.csv')
...
class ObstacleMap:
    ...
    def compute_cost(self, x: torch.Tensor, inital_state=None):
        """
        Compute collision cost for trajectories with robot radius.
        x: (batch, traj_len, dim)
        """

        # Use first state if initial not given
        ...
        # Inflate map (robot radius)
        robot_radius = 1.0  # meters
        radius_occ = int(round(robot_radius / self._cell_size))

        # Precompute inflated map once
        if not hasattr(self, "_inflated_map"):
            import torch.nn.functional as F

            kernel_size = 2 * radius_occ + 1
            kernel = torch.ones((1, 1, kernel_size, kernel_size), device=self._device)

            map_tensor = self._map_torch.unsqueeze(0).unsqueeze(0)  # (1,1,H,W)
            ...
            # Binary inflated obstacle map
            self._inflated_map = (inflated > 0).float().squeeze()
        ...
        detect_range_occ = torch.tensor(
            self._detect_range, device=self._device, dtype=self._dtype
        ) / self._cell_size
        detect_range_occ = torch.round(detect_range_occ).long()
        ...
        is_out_of_bound = torch.logical_or(
            torch.logical_or(
                x_occ[..., 0] < 0, x_occ[..., 0] >= self._inflated_map.shape[0]
            ),
            torch.logical_or(
                x_occ[..., 1] < 0, x_occ[..., 1] >= self._inflated_map.shape[1]
            ),
        )
        ...
        range_occ = ((x_occ[..., 0] - inital_x_occ[0]) ** 2 +
                    (x_occ[..., 1] - inital_x_occ[1]) ** 2) ** 0.5

        is_out_of_range = (range_occ >= detect_range_occ)


        # Collision lookup (TRUE collision, NEVER modified)
        collisions = self._inflated_map[x_occ[..., 0], x_occ[..., 1]]

        # out-of-bound = true collision
        collisions = collisions.clone()
        collisions[is_out_of_bound] = 1.0


        # visualization only (separate copy)
        range_render = collisions.clone()

        # show out-of-range as visible but NOT removing collision
        range_render[is_out_of_range] = 0.0
    ...
    def clear_obstacles(self):
        ...
        if hasattr(self, "_inflated_map"):
            del self._inflated_map
